...
public final class ReflectionHelper {
			...
			if ((access == Opcodes.ACC_PUBLIC) && methodDesc.equals(desc)) { // public void xxx(NBTTagCompund)
				return new EmptyMethodVisitor() {
					...
					public void visitMethodInsn(int opcode, String owner, String name, String desc) {
						if ((opcode == Opcodes.INVOKEVIRTUAL) && compName.equals(owner) && desc.startsWith("(Ljava/lang/String")) { // nbttagcompund.xxx(String,xxx
	...
	private static void getBlockPositionConstructor() {
		try {
			blockPositionConstructor = MinecraftReflection.getBlockPositionClass().getConstructor(int.class, int.class, int.class);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	private static void getCommandSenderGetWorldMethod() {
		commandSenderGetWorldMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_ABSTRACT, MinecraftReflection.getMinecraftClass("ICommandListener"), MinecraftReflection.getNmsWorldClass());
	}

	private static void getCraftEntityGetNMSEntityMethod() {
		craftEntityGetNMSEntityMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC, MinecraftReflection.getCraftEntityClass(), MinecraftReflection.getEntityClass());
	}

	private static void getCraftProxiedCommandSenderGetHandlerMethod() {
		craftProxiedCommandSenderGetHandlerMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC, MinecraftReflection.getCraftBukkitClass("command.ProxiedNativeCommandSender"), MinecraftReflection.getMinecraftClass("ICommandListener"));
	}

	private static void getCraftWorldGetTileEntityMethod() {
		craftWorldGetTileEntityMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC, MinecraftReflection.getCraftWorldClass(), MinecraftReflection.getTileEntityClass(), int.class, int.class, int.class);
	}

	private static void getCraftWorldNMSWorldField() {
		craftWorldNMSWorldField = Accessors.getFieldAccessor(MinecraftReflection.getCraftWorldClass(), MinecraftReflection.getWorldServerClass(), true).getField();
	}
	...
	private static void getEntityGetCommandBlockLogicMethod() {
		entityGetCommandBlockLogicMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC, MinecraftReflection.getMinecraftClass("EntityMinecartCommandBlock"), MinecraftReflection.getMinecraftClass("CommandBlockListenerAbstract"));
	}

	private static void getEntityGetUUIDMethod() {
		entityGetUUIDMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC, MinecraftReflection.getEntityClass(), UUID.class);
	}
	...
	private static void getGetRemoteControlCommandListenerMethod() {
		getRemoteControlCommandListenerMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, MinecraftReflection.getMinecraftClass("RemoteControlCommandListener"), MinecraftReflection.getMinecraftClass("RemoteControlCommandListener"));
	}

	private static void getGettingEntityByUUIDMethod() {
		getEntityByUUIDMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC, MinecraftReflection.getMinecraftServerClass(), MinecraftReflection.getEntityClass(), UUID.class);
	}
	...
	private static void getNBTReadMethod() {
		nbtRead = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, MinecraftReflection.getNbtCompressedStreamToolsClass(), MinecraftReflection.getNBTCompoundClass(), InputStream.class);
	}

	private static void getNBTWriteMethod() {
		nbtWrite = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, MinecraftReflection.getNbtCompressedStreamToolsClass(), void.class, MinecraftReflection.getNBTCompoundClass(), OutputStream.class);
	}
	...
	private static void getSelectingEntityMethod() {
		selectingEntitiesMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, MinecraftReflection.getMinecraftClass("CommandAbstract"), List.class, MinecraftReflection.getMinecraftClass("ICommandListener"), String.class);
	}
	...
	private static void getServerMethod() {
		getServerMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, MinecraftReflection.getMinecraftServerClass(), MinecraftReflection.getMinecraftServerClass());
	}

	private static void getServerWorldsArrayField() {
		serverWorldsArray = NormalFieldFinder.findField(Opcodes.ACC_PUBLIC, MinecraftReflection.getMinecraftServerClass(), getArrayClass(MinecraftReflection.getWorldServerClass()));
	}

	private static void getServerWorldsField() {
		serverWorlds = NormalFieldFinder.findField(Opcodes.ACC_PUBLIC, MinecraftReflection.getMinecraftServerClass(), List.class);
	}
	...
	private static void getTileGetCommandBlockLogicMethod() {
		tileGetCommandBlockLogicMethod = NormalMethodFinder.findMethod(Opcodes.ACC_PUBLIC, MinecraftReflection.getMinecraftClass("TileEntityCommand"), MinecraftReflection.getMinecraftClass("CommandBlockListenerAbstract"));
	}
	...
	private static void getWorldNotifyMethod() {
		try {
			// TODO: Use ASM to find method if you can
			worldNotifyMethod = MinecraftReflection.getNmsWorldClass().getMethod("notify", MinecraftReflection.getBlockPositionClass());
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	public static List<?> selectEntities(CommandSender sender, String expression) {
		...
		Object[] newWorldsArray = (Object[]) Array.newInstance(MinecraftReflection.getWorldServerClass(), worlds.size());
