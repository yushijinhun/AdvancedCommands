...
public class FunctionSqr extends Function {
	...
	public Var call(Var[] args, FunctionContext context) {
		...
		if ((val instanceof Byte) || (val instanceof Short) || (val instanceof Integer)) {
			...
			return new Var(context.getCommandContext().getDataTypes().get("int"), num * num);
		} else if (val instanceof Long) {
			long num = ((Number) val).longValue();
			return new Var(context.getCommandContext().getDataTypes().get("long"), num * num);
		} else if (val instanceof Float) {
			float num = ((Number) val).floatValue();
			return new Var(context.getCommandContext().getDataTypes().get("float"), num * num);
		} else if (val instanceof Double) {
			double num = ((Number) val).doubleValue();
			return new Var(context.getCommandContext().getDataTypes().get("double"), num * num);
		}
