...
public class NBTHandlerImpl implements NBTHandler {

	public final Register<NBTSource> sources;
	public final Set<String> nbtTypes = new LinkedHashSet<>();

	private CommandContext commandContext;

	public NBTHandlerImpl(CommandContext commandContext) {
		this.commandContext = commandContext;
		sources = new RegisterImpl<>("NBTSource", commandContext.getLogger());

		sources.register(new NBTSourceEntity());
		sources.register(new NBTSourceTile());
	...
	public NbtBase<?> createTag(String name, Var data) {
		if (name.equals("BYTE")) {
			return NbtFactory.ofWrapper(NbtType.TAG_BYTE, "", data.getValue() instanceof Boolean ? ((Boolean) data.getValue() ? (byte) 1 : (byte) 0) : (Byte) data.getValue());
	...
	@Override
	public Register<NBTSource> getNBTSources() {
		return sources;
	}

	@Override
	public Set<String> getNBTTypes() {
		return nbtTypes;
	}
