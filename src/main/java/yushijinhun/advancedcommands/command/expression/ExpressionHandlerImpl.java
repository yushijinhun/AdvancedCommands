...
public class ExpressionHandlerImpl implements ExpressionHandler {
	...
	public ExpressionHandlerImpl(CommandContext commandContext) {
		...
		for (int i = 0; i < exps.length; i++) {
			...
			if (isOp(str)) {
				if (str.equals("(") || str.equals("[")) {
					...
					while (!stack.isEmpty() && ((isLeftAssoc(stack.peek()) && (getPriority(str) <= getPriority(stack.peek()))) || (!isLeftAssoc(stack.peek()) && (getPriority(str) < getPriority(stack.peek()))))) {
						result.add(stack.pop());
					}
					stack.push(str);
				}
				...
				if (str.equals("null")) {
					...
					if (str.endsWith("\"")) {
						result.add(new VarWarpperConstant(new Var(commandContext.getDataTypes().get("string"), str.substring(1, str.length() - 1))));
					} else {
						throw new ExpressionHandlingException("Unmatched \"");
					}
					...
					try {
						...
						case 's':
						case 'S':
							constant = new Var(commandContext.getDataTypes().get("short"), Short.valueOf(spilted, radix));
							break;
						...
						default:
							if (str.contains(".")) {
								...
								constant = new Var(commandContext.getDataTypes().get("int"), Integer.valueOf(str, radix));
							}
							break;
						}
